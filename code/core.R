...
#' Read data
#+ cache = TRUE
#path.to.input <- "/home/mp316/bio234/BIO234FinalProject-master/input"
...
levels <- levels(sampdat$population)
...
trans <-sapply(levels, function(t){ #separate matrices based on groups
    ...
    tP <- 1-rowSums(sameState[,idx])/sum(idx) #transition probabilities
    #tP <- tP[1:(length(tP)-1)] #remove last row
    tP <- tP[-length(tP)] #remove last row
